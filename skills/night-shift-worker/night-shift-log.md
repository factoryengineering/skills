...
**`close_console` is the last log step of every run, including a run that stops early.** The server caps one identity at 16 open consoles, and every routine in a practice runs as that same identity, so each run that leaves its console open takes a slot the next one needs. **The pool does heal on its own, and far too slowly to lean on.** A console releases its slot only after a full recovery window in which nothing touches it — seven days, as the server's own `list_consoles` description states — and a console past it is reported `expired` rather than gone. Consoles idle for five days have been seen still holding theirs. A practice that dispatches a few issues a night reaches 16 long before its oldest leak expires, so recovery never arrives in time to help, and every run from then on works through the reuse path below instead of opening a console of its own.
